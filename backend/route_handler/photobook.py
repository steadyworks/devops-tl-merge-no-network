...
from backend.lib.websocket.codec import WebSocketCodec
...
class NewPhotobookRequest(BaseModel):
    ...
    user_gift_recipients: Optional[str] = None
...
class NewPhotobookResponse(BaseModel):
    photobook_id: UUID
    photobook_preview_url: Optional[str]
...
class PhotobookPatchRequest(BaseModel):
    ...
    open_param: Optional[dict[str, str]] = None
...
class CommentResponse(BaseModel):
    ...
    def from_raw_comment(
        ...
        user: DAOUsers | None = next(
            (u for u in users if u.id == comment.user_id), None
        )
        return cls(
...
class PhotobooksFullPublicResponse(PhotobooksOverviewResponse):
    # Hide these private info
    shares: list[SharesOverviewResponse] = Field(
        default_factory=list[SharesOverviewResponse], exclude=True
    )
    share_channels: list[ShareChannelsOverviewResponse] = Field(
        default_factory=list[ShareChannelsOverviewResponse], exclude=True
    )
    unfulfilled_payments: list[PaymentsOverviewResponse] = Field(
        default_factory=list[PaymentsOverviewResponse], exclude=True
    )
...
class PhotobookMetadataResponse(BaseModel):
    title: str


class PhotobookByShareSlugResponse(BaseModel):
...
class EditPageRequest(BaseModel):
    ...
    new_lightweight_title: Optional[str] = None
...
class PhotobookAPIHandler(RouteHandler):
    def register_routes(self) -> None:
        ...
        self.route(
            "/api/photobook/{photobook_id}/metadata",
            "get_photobook_metadata_by_id",
            ["GET"],
        )
        self.route(
    ...
    async def photobook_new(
            ...
            async with safe_transaction(
                ...
                photobook: DAOPhotobooks = await DALPhotobooks.create(
                    ...
                    DAOPhotobooksCreate(
                        ...
                        user_gift_recipient=payload.user_gift_recipients,
                        thumbnail_asset_id=None,
                        deleted_at=None,
                        status_last_edited_by=None,
                        background_color_palette="default",
                        background=BackgroundType.COLOR,
                        open_style=OpenStyle.TWOD_ENVELOPE,
                        open_param={  # Is there a better way to set this?
                            "palette": "neutral"
                        },
        ...
        return NewPhotobookResponse(
            photobook_id=photobook.id,
            photobook_preview_url=f"/photobook/{photobook.id}",
        )
    ...
    async def get_photobook_by_id(
        ...
        async with self.app.new_db_session() as db_session:
            ...
            first_gift_if_exists, sender_name = None, None
            if shares:
                gift_results = await DALGiftcards.list_all(
                    db_session,
                    {
                        "share_id": (
                            FilterOp.IN,
                ...
                if gift_results:
                    first_gift_if_exists = GiftcardsOverviewResponse.from_dao(
                        gift_results[0]
                    )
                sender_name = shares[0].sender_display_name

            return PhotobookByShareSlugResponse(
                photobook=photobook_resp,
                giftcard=first_gift_if_exists,
    ...
    @enforce_response_model
    @posthog_capture()
    async def get_photobook_metadata_by_id(
        self,
        request: Request,
        photobook_id: UUID,
    ) -> PhotobookMetadataResponse:
        async with self.app.new_db_session() as db_session:
            photobook = await DALPhotobooks.get_by_id(db_session, photobook_id)
            if photobook is None:
                raise HTTPException(status_code=404, detail="Photobook not found")

            return PhotobookMetadataResponse(title=photobook.title)
        ...
        async with self.app.new_db_session() as db_session:
            ...
            async with safe_commit(db_session):
                ...
                for edit in payload.edits:
                    ...
                    if edit.new_user_message is not None:
                        update_fields["user_message"] = edit.new_user_message
                    if edit.new_lightweight_title is not None:
                        update_fields["page_lightweight_title"] = (
                            edit.new_lightweight_title
                        )
                    # Only include fields that are valid for DAOPagesUpdate
                    allowed_fields = {"user_message", "page_lightweight_title"}
                    filtered_update_fields = {
                        k: v for k, v in update_fields.items() if k in allowed_fields
                    }
                    if filtered_update_fields:
                        update_map[edit.page_id] = DAOPagesUpdate(
                            **filtered_update_fields
                        )
                await DALPages.update_many_by_ids(db_session, update_map)

            # 3. Return updated photobook and its pages
            return await PhotobooksFullPublicResponse.rendered_from_dao(
                photobook, db_session, self.app.asset_manager
            )
    ...
    async def photobook_create_comment(
        ...
        # logged_in_user_id = request_context.user_id
        user_id: UUID | None = None
        try:
            request_context: RequestContext = await self.get_request_context(request)
            user_id = request_context.user_id
        except Exception:
            pass  # Allow unauthenticated users to comment as guests
        ...
        async with self.app.new_db_session() as db_session:
            async with safe_commit(db_session):
                ...
                user: DAOUsers | None = (
                    await DALUsers.get_by_id(db_session, user_id) if user_id else None
                )

                return CreateCommentResponse(
                    comment=CommentResponse.from_raw_comment(
                        comment, [user] if user else []
                    )
                )
    ...
    async def photobook_status_ws(self, websocket: WebSocket) -> None:
        ...
        logging.debug(f"[WS] Photobook status client connected user_id={user_id}")

        codec = WebSocketCodec(websocket)
        poll_task: asyncio.Task[None] | None = None

        try:
            async for msg in codec.aiter_recv(
                log_prefix=f"[WS photobook user_id={user_id}]",
                keepalive=True,  # ignore malformed frames; keep the socket up
            ):
                try:
                    match msg:
                        case PhotobookStatusSubscribeMessage(payload=payload):
                            ...
                            # Start poller that streams updates over this websocket
                            poll_task = asyncio.create_task(
                                self._poll_and_stream_photobook_status(
                                    codec, user_id, payload.photobook_id
                                )
                            )

                        # Ignore other events on this endpoint (keep connection open)
                        case AssetUploadStatusMessage():
                            logging.debug(
                                f"[WS photobook user_id={user_id}] "
                                f"Ignoring ASSET_UPLOAD_STATUS_UPDATE on status socket"
                            )

                        case _:
                            logging.warning(
                                f"[WS photobook user_id={user_id}] Unknown event: "
                                f"{getattr(msg, 'event', None)}"
                            )

                except Exception as e:
                    logging.warning(f"[WS photobook] handler error: {e}")
                    continue

        except WebSocketDisconnect:
            logging.info(f"[WS] Photobook WS disconnected user_id={user_id}")
        finally:
            if poll_task and not poll_task.done():
                poll_task.cancel()
                with suppress(Exception):
    ...
    async def _poll_and_stream_photobook_status(
        ...
        while True:
            ...
            if pb is None:
                await self._send_pb_error(codec, photobook_id, "photobook not found")
                return
            if pb.user_id != user_id:
                await self._send_pb_error(codec, photobook_id, "forbidden")
                return
            ...
            if pb_status != last_status:
                ...
                try:
                    await codec.send(
                        PhotobookStatusUpdateMessage(
                            event=ServerToClientWebSocketEventType.PHOTOBOOK_STATUS_UPDATE,
